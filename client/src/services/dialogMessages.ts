...
class DialogMessagesService {
  ...
  getAll = async (paginationOptions: IPaginationOptions & { friendId: string }) => {
    const response = await customFetch(
      `${this.url}?friendId=${paginationOptions.friendId}&page=${paginationOptions.page}&limit=${paginationOptions.limit}`,
      {
        cache: "no-store"
      }
    );
